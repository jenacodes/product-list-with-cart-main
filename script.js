...
const updateQuantity = (product, change) => {
  ...
  if (cartItem) {
    ...
    const productElement = document.querySelector(
      `.full-grid[data-id="${product.id}"]`
    );
    if (productElement) {
      const itemQuantityElement =
        productElement.querySelector(".item-quantity");
...
const addToCartHtml = () => {
  ...
  carts.forEach((cartItem) => {
    ...
    cartItemElement.innerHTML = `
                <div class="cart-quantity">
                    ...
                    <div class="quantity-wrapper">
                        ...
                        <span class="each-item">@$${product?.price.toFixed(
                          2
                        )}</span>
    ...
    removeItemsButtons.addEventListener("click", () =>
...
function updateModal() {
  const modalContentWrapper = modalContainer.querySelector(
...
const reflectCartState = () => {
  carts.forEach((cartItem) => {
    const productElement = document.querySelector(
      `.full-grid[data-id="${cartItem.product.id}"]`
    );
    if (productElement) {
